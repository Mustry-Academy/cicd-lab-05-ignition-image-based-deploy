...
```
push to main ──▶ build  ──▶ :sha-abc1234 ──▶ deploy to dev  (test it here)
                                  │
tag v0.1.0 ────▶ promote ─────────┘ re-tag ──▶ :v0.1.0 + :prod ──▶ deploy to prod
                 (no rebuild — same digest)
```

`release.yml` does **not** rebuild on a tag. It runs `docker buildx imagetools create` to copy the manifest of the already-tested `:sha-<short>` image onto new tags (`:v0.1.0`, `:prod`). Server-side, no layers moved. Prove it:
